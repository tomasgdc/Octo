...
namespace Renderer
{
	void RenderPassFullScreen::UpdateUniformBufferData()
	{
		// Update matrices
	...
	void RenderPassFullScreen::Init()
	{
		LoadShaders("triangle.vert.spv", "triangle.frag.spv");
		CreatePipelineLayout("RenderPassFullScreen_PipelineLayout");
		CreateRenderPass("RenderPassFullScreen_RenderPass");
		CreateFramBuffer("RenderPassFullScreen_FrameBuffer");
		CrreateBufferLayout("RenderPassFullScreen_BufferLayout");
		CreatePipeline("RenderPassFullScreen_Pipeline");
		...
		m_StagingBufferVerticesRef = CreateBuffer("RenderPassFullScreenVertBuffer", VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, vertData.data(), static_cast<uint32_t>(vertData.size() + sizeof(drawVert)));
		m_StagingBufferIndicesRef = CreateBuffer("RenderPassFullScreenIndexBuffer", VK_BUFFER_USAGE_INDEX_BUFFER_BIT, indexBuffer.data(), indexBufferSize);
		m_UniformBufferRef = CreateUniformBuffer("RenderPassFullScreenUniformBuffer", VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, &m_UboData, sizeof(m_UboData));

		m_DrawCallRef = CreateDrawCall("RenderPassFullScreenDrawCall", indexBuffer.size());
	}

	void RenderPassFullScreen::Destroy()
	{

	}

	void RenderPassFullScreen::Render(float dt, float width, float height)
	{
		VkClearValue clearValues[2];
		clearValues[0].color = { { 0.5f, 0.5f, 0.5f, 1.0f } };
		clearValues[1].depthStencil = { 1.0f, 0 };

		Renderer::Vulkan::RenderSystem::BeginRenderPass(m_RenderPassRef, m_FrameBufferRefs[Renderer::Vulkan::RenderSystem::backBufferIndex % m_FrameBufferRefs.size()], VK_SUBPASS_CONTENTS_INLINE, 2, clearValues);
		Renderer::Vulkan::RenderSystem::DispatchDrawCall(m_DrawCallRef, Renderer::Vulkan::RenderSystem::GetPrimaryCommandBuffer(), width, height);
		Renderer::Vulkan::RenderSystem::EndRenderPass();
	}

	bool RenderPassFullScreen::LoadShaders(const std::string& vertShader, const std::string& fragShader)
	{
		//Create GPU Resource
	...
	void RenderPassFullScreen::CreatePipelineLayout(const std::string& pipelineLayoutName)
	{
	...
	void RenderPassFullScreen::CreateRenderPass(const std::string& renderPassName)
	{
	...
	void RenderPassFullScreen::CreateFramBuffer(const std::string& frameBufferName)
	{
	...
	void RenderPassFullScreen::CrreateBufferLayout(const std::string& bufferLayoutName)
	{
	...
	void RenderPassFullScreen::CreatePipeline(const std::string& pipelineName)
	{
	...
	DOD::Ref RenderPassFullScreen::CreateBuffer(const std::string& name, VkBufferUsageFlagBits usage, void* bufferData, int32_t bufferSize)
	{
	...
	DOD::Ref RenderPassFullScreen::CreateUniformBuffer(const std::string& name, VkBufferUsageFlagBits usage, void* bufferData, int32_t bufferSize)
	{
	...
	DOD::Ref RenderPassFullScreen::CreateDrawCall(const std::string& name, int32_t indexBufferSize)
	{
